...
const KillRing = @import("kill_ring").KillRing;
        ...
        while (self.running) {
            ...
            while (true) {
                if (try input.readKeyFromReader(input_reader)) |key| {
                    keys_processed += 1;
                    // ミニバッファモード中、確認待ちモード中、履歴ナビゲーション中はプロンプトを維持
                    if (!self.isMinibufferMode() and self.mode != .query_replace_confirm and self.mode != .recent_files) {
                        self.getCurrentView().clearError();
                    }
