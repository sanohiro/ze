...
test "detectLanguage by extension" {
    ...
    try testing.expectEqual(&lang_python, detectLanguage("test.py", null));
    try testing.expectEqual(&lang_zig, detectLanguage("main.zig", null));
    try testing.expectEqual(&lang_shell, detectLanguage("script.sh", null));
    try testing.expectEqual(&lang_c, detectLanguage("main.c", null));
    try testing.expectEqual(&lang_rust, detectLanguage("lib.rs", null));
}

test "detectLanguage by filename" {
    const testing = std.testing;

    try testing.expectEqual(&lang_makefile, detectLanguage("Makefile", null));
    try testing.expectEqual(&lang_dockerfile, detectLanguage("Dockerfile", null));
    try testing.expectEqual(&lang_shell, detectLanguage(".bashrc", null));
}

test "detectLanguage by shebang" {
    const testing = std.testing;

    try testing.expectEqual(&lang_shell, detectLanguage("script", "#!/bin/bash\necho hello"));
    try testing.expectEqual(&lang_python, detectLanguage("script", "#!/usr/bin/env python\nprint('hello')"));
    try testing.expectEqual(&lang_javascript, detectLanguage("script", "#!/usr/bin/env node\nconsole.log('hi')"));
}
...
test "findCommentStart - basic" {
    ...
    // 位置11から "//" が始まる
    try testing.expectEqual(@as(?usize, 11), lang_cpp.findCommentStart("int x = 0; // comment"));
    try testing.expectEqual(@as(?usize, 0), lang_cpp.findCommentStart("// full line comment"));
    try testing.expectEqual(@as(?usize, null), lang_cpp.findCommentStart("int x = 0;"));

    // Python
    // "x = 10  # comment"
    //  01234567 (位置8が#)
    try testing.expectEqual(@as(?usize, 8), lang_python.findCommentStart("x = 10  # comment"));
    try testing.expectEqual(@as(?usize, 0), lang_python.findCommentStart("# comment"));
}
...
test "findCommentStart - string skip" {
    ...
    // 位置28から "//" が始まる
    try testing.expectEqual(@as(?usize, 28), lang_cpp.findCommentStart("char* s = \"// not comment\"; // real"));

    // Python: 文字列内の # はコメントじゃない
    // "s = \"# not comment\""  (19文字、0-18)
    try testing.expectEqual(@as(?usize, null), lang_python.findCommentStart("s = \"# not comment\""));
    // "s = \"# not comment\" # real"
    //  0         1         2
    //  01234567890123456789012345
    // 位置20から "#" が始まる
    try testing.expectEqual(@as(?usize, 20), lang_python.findCommentStart("s = \"# not comment\" # real"));
