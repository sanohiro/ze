...
**簡易正規表現検索**: パターンに `. * + ? [ ] ^ $ \` が含まれると自動で正規表現として扱われます。
...
`M-x` でコマンドプロンプト `:` が開き、短いコマンドを入力して実行。

### 設計思想

- **短いコマンド名**: `set-tab-width` ではなく `tab`
- **引数なしで現在値表示**: `tab` → `tab: 4`
- **引数ありで設定変更**: `tab 2` → タブ幅を2に変更
- **Unixに任せる**: エンコーディング変換は `iconv`、改行コードは `dos2unix`

### コマンド一覧

| コマンド | 説明 | 例 |
|---------|------|-----|
| `line` | 指定行へ移動 | `line 100` |
| `tab` | タブ幅の表示/設定 | `tab` / `tab 2` |
| `indent` | インデントスタイル | `indent` / `indent space` / `indent tab` |
| `mode` | 言語モードの表示/設定 | `mode` / `mode python` |
| `revert` | ファイル再読み込み | `revert` |
| `key` | 次に押すキーの説明 | `key` → C-x C-s → "C-x C-s: save" |
| `ro` | 読み取り専用切り替え | `ro` |
| `?` | コマンド一覧表示 | `?` |
...
```
M-x line 100     → 100行目へジャンプ
M-x tab          → "tab: 4" (現在値)
M-x tab 2        → タブ幅を2に変更
M-x mode         → "mode: Zig" (現在値)
M-x mode py      → Pythonモードに変更（部分マッチ）
M-x revert       → ファイル再読み込み（未保存なら警告）
M-x key          → "Press key:" → C-x C-s → "C-x C-s: save"
M-x ro           → 読み取り専用ON/OFF切り替え
M-x ?            → コマンド一覧
```
...
| 機能 | 理由 |
|------|------|
| `eol` (改行コード変更) | 危険。`dos2unix` / `unix2dos` を使え |
| `encoding` (文字コード変更) | 危険。`iconv` を使え |
| `count` (文字数カウント) | `M-\| % \| wc` で十分 |
...
zeは**最小限の色使い**で、どんな端末でも動作することを優先します。

### 設計方針

1. **基本ANSI色のみ使用**: 16色モード（SGR 30-37, 90-97）で動作
2. **24bit色は使わない**: 端末互換性を最大化
3. **シンタックスハイライトは実装しない**: zeは編集ツール、IDEではない
4. **色が崩れても操作可能**: 色は補助情報、必須ではない

### 現在使用している色

| 要素 | ANSIコード | 見た目 | 用途 |
|------|-----------|--------|------|
| 通常テキスト | なし（デフォルト） | 端末の前景色 | 本文 |
| 行番号 | `\x1b[90m` | Bright Black (Gray) | 視覚的に控えめに |
| コメント | `\x1b[90m` | Bright Black (Gray) | 本文より目立たなく |
| 検索マッチ | `\x1b[7m` | 反転表示 | 目立たせる |
| ステータスバー | `\x1b[7m` | 反転表示 | 区切りを明確に |

### なぜシンタックスハイライトを実装しないのか

- **zeの目的**: サーバーの設定ファイルをサクッと編集
- **典型的な使用時間**: 数秒〜数分
- **ハイライトの価値**: 短時間編集では低い
- **実装コスト**: 言語ごとのパーサー、テーマ管理、24bit色対応...
- **結論**: 複雑さに見合わない。本格的な編集はVSCodeで。

### コメントのグレー表示

唯一の「色分け」として、コメント行をグレー表示します：

- **目的**: コードとコメントの視覚的区別
- **実装**: 単純な行頭パターンマッチ + ブロックコメント追跡
- **対応**: 48言語の行コメント・ブロックコメント

これは「シンタックスハイライト」ではなく「コメントの視認性向上」です。
