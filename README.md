...
- [x] **パフォーマンス最適化**
  ...
  - [x] PieceIterator（効率的なイテレーション）
  - [x] LineIterator（O(n²)の回避）
  - [x] 差分描画の基礎（clear()削除、1 syscallレンダリング）
  - [x] インライン関数とコンパイル時最適化
...
### 最優先で対処が必要
1. **UTF-8非対応**: 現状ASCIIのみ、日本語が壊れる
2. **カーソル位置の不正確さ**: 行末を超えて移動可能
3. **エラーハンドリング**: `try`で即終了、ユーザーにメッセージなし

### Phase 2で対処
4. **Undo/Redoのメモリ効率**: piece配列全体をクローン
5. **ファイルロード**: readToEndAllocではなくmmap
6. **完全な差分描画**: dirty lineトラッキング未実装
