...
```
ファイル読み込み時:
  外部ファイル (任意のエンコーディング・改行コード)
      ↓ 検出・正規化
  内部バッファ (UTF-8 + LF)

編集中:
  すべての操作は UTF-8 + LF で実行
  - カーソル移動、挿入、削除、検索、Undo/Redo
  - エンコーディングや改行コードを意識する必要なし

ファイル保存時:
  内部バッファ (UTF-8 + LF)
      ↓ 変換
  外部ファイル (元のエンコーディング・改行コード)
```

**現在の対応状況** (Phase 2完了):
- ✅ **UTF-8**: 完全対応（内部固定）
- ✅ **改行コード**: LF/CRLF 自動検出・保持
  - 読み込み時に CRLF → LF に正規化
  - 保存時に検出した改行コードで出力
  - ステータスラインに表示 (LF/CRLF)
- 🔜 **将来対応予定** (Phase 8+):
  - Shift_JIS、EUC-JP 等の自動検出・変換
  - 同様に読み込み時 UTF-8 変換、保存時元エンコーディング変換
...
### Phase 2: 安全性と基本編集 (v0.2) ✅ **完了 (2024-12-05)**
**目標: SSH先で安心して使えるレベル**
...
- [x] **拡張ステータス情報** ✅
  ...
  - [x] 行番号 / カラム表示
  - [x] 改行コード表示（LF / CRLF）
  - [x] エンコーディング表示（UTF-8 固定）

#### 改行コード対応
- [x] **CRLF 対応** ✅
  - [x] 開いたときに LF/CRLF を自動検出
  - [x] 読み込み時に CRLF → LF 正規化
  - [x] 保存時に元の改行コードで出力
  - [x] 内部表現は常に UTF-8 + LF
